...
func imagesInRow (row: SkullRow) -> ITunesImages? {
  if let img100 = row["img100"] as? String {
    if let img30 = row["img30"] as? String {
      if let img600 = row["img600"] as? String {
        if let img60 = row["img60"] as? String {
          return ITunesImages(
            img100: img100
          , img30: img30
          , img600: img600
...
func resultFromRow (row: SkullRow, df: SQLDates) -> SearchResult? {
  if let author = row["author"] as? String {
    if let feed = row["feed"] as? String {
